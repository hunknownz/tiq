...
// @Accept json
// @Produce json
// @Param topic body model.TopicConfig true "topic config"
// @Success 202
// @Failure 500,400 {object} model.HTTPError
...
func (h *HTTPHandler) CreateTopic(c *gin.Context) {
	...
	var topicConfig model.TopicConfig
	if err := c.BindJSON(&topicConfig); err != nil {
		_ = c.Error(cerror.ErrAPIInvalidParam.Wrap(err))
		return
	}

	log.Info("Create Topic successfully!", zap.String("topic", topicConfig.TopicName), zap.String("table", topicConfig.TableName))
	c.Status(http.StatusAccepted)
}